...
<script>
	...
	$: bestOpenings = {
		white: data
			?.filter((opening) => opening.gamesPlayed >= 3)
			.reduce(
				(best, current) => {
					if (!best.white || current.whiteWinRate > best.white.winRate) {
						return {
							...
							white: {
								...
								winRate: current.whiteWinRate,
								games: current.whiteGames
							}
						};
					}
					return best;
				...
				{ white: null }
			)?.white || { opening: 'No data', winRate: 0, games: 0 },
		black: data
			?.filter((opening) => opening.gamesPlayed >= 3)
			.reduce(
				(best, current) => {
					if (!best.black || current.blackWinRate > best.black.winRate) {
						return {
							...
							black: {
								opening: current.opening,
								winRate: current.blackWinRate,
								games: current.blackGames
							}
						};
					}
					return best;
				...
				{ black: null }
			)?.black || { opening: 'No data', winRate: 0, games: 0 }
	};
</script>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
	<!-- Best White Opening -->
	<div
		class="flex flex-col items-center justify-center p-4 rounded-lg bg-surface-100-800-token/30 backdrop-blur-sm"
	>
	...
	<!-- Best Black Opening -->
	<div
		class="flex flex-col items-center justify-center p-4 rounded-lg bg-surface-900-50-token/30 backdrop-blur-sm"
	>
		<div class="flex flex-col items-center space-y-2 w-full">
			<div class="flex items-center justify-center w-12 h-12 rounded-full bg-white/10 mb-2">
				<svg
